...
from models import AudioRNNModel, MLPReg, MLPClass
...
for seed in seed_list:
    if task != "type":
        ...
        # model_params = {'model_name': 'emo18',
        #                 'input_size': feat_dimensions,
        #                 'num_outs': len(classes)}
        # model = get_model('audio', **model_params).to(dev)
        model = AudioRNNModel(input_size=feat_dimensions, 
                              num_outs=len(classes), 
                              model_name='emo18').to(dev)
    else:
